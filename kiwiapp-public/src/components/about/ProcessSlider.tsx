        ...
        <div className="relative flex flex-col">
              ...
              {processSliderData.map((slide, i) => (
                <div className={cn("w-full !flex justify-center transition-opacity duration-300", i !== slideIndex && "max-sm:!opacity-0", i !== slideIndex && i !== slideIndex + 1 && "max-xl:!opacity-0", i !== slideIndex && i !== slideIndex + 1 && i !== slideIndex + 2 && "xl:!opacity-0")} key={slide.description}>
                  <div className="relative flex flex-col pt-8 max-w-[340px] xl:max-w-[374px]">
                    <div className="absolute top-0 left-0 -translate-x-[50%] rounded-full flex items-center justify-center border-2 border-evergreen-800 text-3xl leading-[1] font-bold w-16 h-16 box-border bg-remGreen-400">{i + 1}</div>
          ...
          <div className="flex-1 flex justify-center">
            <div className="mt-3 sm:mt-12 items-center gap-5 flex">
              {/* @ts-expect-error react-slick */}
              <button disabled={!smallScreen && !mediumScreen && slideIndex === 0} onClick={() => sliderRef.slickGoTo(slideIndex - 1)} className={cn("w-16 h-16 flex justify-center items-center rounded-full z-10", !smallScreen && slideIndex === 0 ? "bg-beige-400 cursor-default" : "bg-remGreen-400 hover:bg-remGreen-500 transition-colors cursor-pointer")}>
                <Image src={ChevronLeftIcon} alt='ChevronLeftIcon'/>
              </button>
              {/* @ts-expect-error react-slick */}
              <button disabled={!smallScreen && !mediumScreen && slideIndex === 1} onClick={() => sliderRef.slickGoTo(slideIndex + 1)} className={cn("w-16 h-16 flex justify-center items-center rounded-full z-10", !smallScreen && !mediumScreen && slideIndex === 1 ? "bg-beige-400 cursor-default" : "bg-remGreen-400 hover:bg-remGreen-500 transition-colors cursor-pointer")}>
